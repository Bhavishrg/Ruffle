...
names = ['party0.txt', 'party1.txt', 'party2.txt', 'client.txt']
...
lines4 = [line.rstrip() for line in fD]
...
for i in range(0, len(lines1)):
    ...
    if "My time" in lines1[i]:
        ...
        num4 = float(lines4[i][startIndex:])

        maxVal = max(num1, num2, num3, num4)
        if state == "P":
    ...
    if "Sent to" in lines1[i]:
        ...
        num3 = float(lines3[i][startIndex:])
        num4 = float(lines4[i][startIndex])
        totalCommunication = totalCommunication + num1 + num2 + num3 + num4
