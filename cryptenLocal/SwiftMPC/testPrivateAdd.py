...
def testSWIFT():
    ...
    a_enc.privateAddition(a_enc, b_enc, 0)

    # Send around commitments of final masks
    ouputCommitments.circulateCommitments(rank, a_enc)
    ...
    #Perform operations
    a_enc.privateAddition(a_enc, b_enc, 1)

    #Verify jmp sends and open commitments
    ouputCommitments.verifyJmpSend(rank)
   
    output = ouputCommitments.openCommitments(rank, a_enc)
