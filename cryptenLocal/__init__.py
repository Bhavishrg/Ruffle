...
import cryptenLocal.common  # noqa: F401
import cryptenLocal.communicator as comm
import cryptenLocal.config  # noqa: F401
import cryptenLocal.mpc  # noqa: F401
import cryptenLocal.nn  # noqa: F401
import cryptenLocal.optim  # noqa: F401
import torch
...
def _setup_prng():
    ...
    #Create global seed for premult
    premult_seed = int.from_bytes(os.urandom(8), "big") - 2 ** 63
    premult_seed = torch.tensor(premult_seed)
    premult_seed = comm.get().broadcast(premult_seed, 0).item()
    ...
    for device in generators["prev"].keys():
        ...
        #Seeds for swift
        prev_seed = hex(abs(prev_seed))
        next_seed = hex(abs(next_seed))
        local_seed = hex(abs(local_seed))
        global_seed = hex(abs(global_seed))
        premult_seed = hex(abs(premult_seed))
        # print(prev_seed, next_seed, local_seed, global_seed)

        if len(prev_seed) < 32:
            dif = 32-len(prev_seed)
            for i in range(0, dif):
                prev_seed = prev_seed+'0'
              
        else:
            prev_seed = prev_seed[0: 32]

        if len(next_seed) < 32:
            dif = 32-len(next_seed)
            for i in range(0, dif):
                next_seed = next_seed+'0'
              
        else:
            next_seed = next_seed[0: 32]

        if len(local_seed) < 32:
            dif = 32-len(local_seed)
            for i in range(0, dif):
                local_seed = local_seed+'0'
              
        else:
            local_seed = local_seed[0: 32]
        
        if len(global_seed) < 32:
            dif = 32-len(global_seed)
            for i in range(0, dif):
                global_seed = global_seed+'0'
              
        else:
            global_seed = global_seed[0: 32]

        if len(premult_seed) < 32:
            dif = 32-len(premult_seed)
            for i in range(0, dif):
                premult_seed = premult_seed+'0'
              
        else:
            premult_seed = premult_seed[0: 32]
        generators["prev"] = prev_seed
        generators["next"] = next_seed 
        generators["local"] = local_seed 
        generators["global"] = global_seed  
        generators["premult"] = premult_seed  
        
        #16 byte seeds
        generators["permCommon"] = permCommon
